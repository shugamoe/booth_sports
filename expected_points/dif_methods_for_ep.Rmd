...
```{r build_vk}
V_k <- KOFF %>% group_by(Yfog) %>% summarise(mean_eoh_net = mean(Net_Score_to_Half))
...
$$
ENP(yrdline = x) = Prob(O\;|\; yrdline = x) * [(ExpPTS_O\; | O\;Score, yrdline = x) - V_k] -
$$
$$
 Prob(D | yrdline = x) * [(ExpPTS_D\; | D Score, yrdline = x) - V_k] + 
$$
$$
[1 - Prob(O|yrdline = x) - Prob(D|yrdline=x)] * (ExpNetPts | Not\;TD,\;Not\;FG,yrdline = x)
$$
